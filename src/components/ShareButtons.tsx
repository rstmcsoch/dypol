...
export function ShareButtons() {
  ...
  const onNativeShare = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const shared = await shareViaNavigator(content);
  ...
  return (
    <div className="flex items-center gap-1.5">
      <button
        ...
        aria-label="Share"
        title="Share"
        className={`${size} grid place-items-center rounded-full bg-primary/15 text-primary hover:bg-primary/25 transition active:scale-90`}
      >
        <Share2 className="h-4 w-4" />
      </button>
      <a
        href={whatsappShareHref(content)}
        target="_blank"
      ...
      <a
        href={telegramShareHref(content)}
        target="_blank"
        rel="noopener noreferrer"
        onClick={(e) => e.stopPropagation()}
